...
export default function ClientCard({ client, region, products, onClick }: ClientCardProps) {
  // Group products by name with counts
  const productCounts = client.products.reduce((acc, p) => {
      ...
      <div className="flex justify-between items-start gap-4">
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-2">
            {displayedProducts.map(([productName, count]) => (
              <span 
                key={productName}
                className="px-3 py-1 bg-[#ccbeac]/20 text-[#0b0b0b] dark:text-[#f9f9f4] rounded-full text-sm"
              >
                {productName} {count > 1 && `(${count})`}
              </span>
            ))}
            {showMore && (
              <span className="px-3 py-1 bg-[#ccbeac]/20 text-[#0b0b0b] dark:text-[#f9f9f4] rounded-full text-sm">
                +{moreCount} more
              </span>
            )}
          </div>
